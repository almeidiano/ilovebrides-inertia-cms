...
import NavigationItemChild from "@/Pages/components/footer/NavigationItemChild.jsx";
import WeddingDistrictiesItemChild from "@/Pages/components/footer/WeddingDistrictiesItemChild.jsx";
// import WeddingDistrictiesItemChild from "@/Pages/components/footer/WeddingDistrictiesItemChild.jsx";

// const WeddingDistrictiesItem = ({
...
//                             updateChildItem, footerData, setFooterData
//                         }) => {
...
//             const items = footerData.navigation.map((navItem) => {
...
// };
//
// export default WeddingDistrictiesItem;

const WeddingDistrictiesItem = ({
                                    item, index, handleCollapse, selectedIndex, deleteNavigationItem,
                                    deleteWeddingChild, createWeddingDataChildren, updateWeddingDistrictiesText,
                                    updateWeddingChildItem, footerData, setFooterData
                                }) => {
    const [editMode, setEditMode] = useState(false);

    const handleSave = () => {
        updateWeddingDistrictiesText(item.text);
        setEditMode(false);
    };
    ...
    const onDragEnd = (result) => {
        ...
        if (result.type === 'child') {
            const childrenList = footerData.weddingDistricties.children;
            const reorderedChildren = reorder(childrenList, result.source.index, result.destination.index);
                ...
                <div>
                    ...
                    {item.children && item.children.length > 0 && (
                        <Collapse in={selectedIndex === index} timeout="auto" unmountOnExit>
                            <List component="div" disablePadding>
                                <div className='flex justify-center'>
                                    <IconButton onClick={() => createWeddingDataChildren(item.id)} aria-label="add">
                                        <AddIcon />
                                    </IconButton>
                                </div>
